...
@pytest.mark.parametrize("file_path, field_name, expected_course_start_date", [
    ("./data/certificate1.docx", "course_start_date", "SEPTEMBER 30, 2024"),
])
def test_course_start_date(file_path, field_name, expected_course_start_date):
    try:
        document_text = docx2txt.process(file_path)
    except Exception as e:
        pytest.fail(f"Failed to process document: {file_path}. Error: {str(e)}")
    
    client_predictions = predict_course_start_date(document_text, field_name)
    ...
    assert predictions_list[0]["text"] == expected_course_start_date, (
        f"Expected company ID '{expected_course_start_date}' but got '{predictions_list[0]['text']}'"
    )